    ...
    for n, ind in enumerate(pop):
        ...
        # don't evaluate if identical phenotype has already been evaluated
            ...
            if pop.gen % save_vxa_every == 0 and save_vxa_every > 0:
                sub.call("cp " + run_directory + "/voxelyzeFiles/" + run_name + "--id_%09i.vxa" % ind.id +
                         " " + run_directory + "/Gen_%04i/" % pop.gen + run_name +
                         "--Gen_%04i--fit_%.08f--id_%09i.vxa" % (pop.gen, ind.fitness, ind.id), shell=True)
        ...
        # otherwise evaluate with voxelyze
            ...
            sub.Popen("./voxelyze  -f " + run_directory + "/voxelyzeFiles/" + run_name + "--id_%09i.vxa" % ind.id,
                      shell=True)
    ...
    while not all_done:
        ...
        if time_waiting_for_fitness > pop.pop_size * time_to_try_again * redo_attempts:
            ...
            for idx in non_analyzed_ids:
                sub.Popen("./voxelyze  -f " + run_directory + "/voxelyzeFiles/" + run_name + "--id_%09i.vxa" % idx,
                          shell=True)
                    ...
                    for ind in pop:
                        if ind.id == this_id:
                            ...
                            if new_run_champ:
                                ...
                                sub.call("cp " + run_directory + "/voxelyzeFiles/" + run_name + "--id_%09i.vxa" %
                                         ind.id + " " + run_directory + "/bestSoFar/fitOnly/" + run_name +
                                         "--Gen_%04i--fit_%.08f--id_%09i.vxa" %
                            ...
                            if pop.gen % save_vxa_every == 0 and save_vxa_every > 0:
                                sub.call("mv " + run_directory + "/voxelyzeFiles/" + run_name + "--id_%09i.vxa" %
                                         ind.id + " " + run_directory + "/Gen_%04i/" % pop.gen +
                                         run_name + "--Gen_%04i--fit_%.08f--id_%09i.vxa" %
                                         (pop.gen, ind.fitness, ind.id), shell=True)
                            else:
                                sub.call("rm " + run_directory + "/voxelyzeFiles/" + run_name + "--id_%09i.vxa" %
                                         ind.id, shell=True)
