...
from softbot import Population
...
class MultiLevelSelectionOptimization(PopulationBasedOptimizer):
    def __init__(self, sim, env, pop):
        ...
        self.number_of_collectives = 4
        self.num_new_collectives = 1
        self.collectives = []


    def assign_collectives(self):
        labels = self.cluster_population()

        collectives = {}
        for unique_label in set(labels):
            collectives[unique_label] = []

        for label,ind in zip(labels, self.pop):
            collectives[label].append(ind)

        for label,inds in collectives.items():
            p = Population(self.pop.objective_dict, self.pop.genotype,
                           self.pop.phenotype, pop_size=len(collectives[label]))
            p.max_id = int(label) * 100000000
            p.individuals = collectives[label]
    ...
    def cluster_population(self):
        ...
        if self.number_of_collectives > 1:
            labels = kmeans.Clustering(self.pop, self.number_of_collectives)
    ...
    def run(self, max_hours_runtime=29, max_gens=3000, num_random_individuals=1, num_env_cycles=0,
            directory="tests_data", name="TestRun", max_eval_time=60, time_to_try_again=30,
            checkpoint_every=100, save_vxa_every=100, save_pareto=False, save_nets=False,
            save_lineages=False, continued_from_checkpoint=False, batch_size=None,
            update_survivors_age=True, max_fitness=None):
        ...
        if not continued_from_checkpoint:  # generation zero
            ...
            self.collectives = self.assign_collectives()

            for label, pop in self.collectives.items():
                self.evaluate(self.sim, self.env[self.curr_env_idx], pop, print_log,
                              save_vxa_every, self.directory,
                              self.name, max_eval_time, time_to_try_again, save_lineages, batch_size)
                self.select(pop)  # only produces dominated_by stats, no selection happening (population not replaced)
                write_gen_stats(pop, self.directory, self.name,
                                save_vxa_every, save_pareto, save_nets,
                                save_lineages=save_lineages)

                print_log.message("Collective: {}, len: {}".format(label, len(pop)))
        ...
        while self.pop.gen < max_gens:
            ...
            for label,pop in self.collectives.items():
                print_log.message("Mutation starts for collective {}".format(label))
                new_children = self.mutate(pop, print_log=print_log)
                print_log.message("Mutation ends: successfully generated %d new children." % (len(new_children)))

                # combine children and parents for selection
                print_log.message("Now creating new population")
                pop.append(new_children)
                for _ in range(self.num_random_inds):
                    print_log.message("Random individual added to population")
                    pop.add_random_individual()
                print_log.message("New population size is %d" % len(pop))
                ...
                # perform selection by pareto fronts
                new_population = self.select(pop)

                # print population to stdout and save all individual data
                print_log.message("Saving statistics")
                write_gen_stats(pop, self.directory, self.name, save_vxa_every, save_pareto, save_nets,
                                save_lineages=save_lineages)

                # replace population with selection
                pop.individuals = new_population
                print_log.message("Population size reduced to %d" % len(pop))
                print_log.message("========================================")


            for label, pop in self.collectives.items():
                print_log.message("Collectives {}, fitness: {}, len: {}".format(
                    label,
                    sum(ind.fitness for ind in pop) / len(pop),
                    len(pop)
                ))

            for _ in range(self.num_new_collectives):
                self._replace_worst_collective(print_log, save_vxa_every, max_eval_time,
                                               time_to_try_again, save_lineages, batch_size)
    ...
    def _replace_worst_collective(self, print_log, save_vxa_every, max_eval_time,
                                  time_to_try_again, save_lineages, batch_size):
        worst_collective_label = self._get_worst_collective()
        worst_collective_size = len(self.collectives[worst_collective_label])
        worst_collective = self.collectives.pop(worst_collective_label)

        new_collective = Population(self.pop.objective_dict, self.pop.genotype,
                                    self.pop.phenotype, pop_size=worst_collective_size)
        new_collective.max_id = worst_collective.max_id

        num_solutions = int(math.ceil(worst_collective_size / len(self.collectives)))
        for _,pop in self.collectives.items():
            pop.sort(key="fitness", reverse=True)

            for best_solution in pop.individuals[:num_solutions]:
                if len(new_collective) < worst_collective_size:
                    clone = copy.deepcopy(best_solution)
                    clone.id = new_collective.max_id

                    new_collective.max_id += 1
                    new_collective.append(clone)

        self.evaluate(self.sim, self.env[self.curr_env_idx], new_collective, print_log,
                    save_vxa_every, self.directory,
                    self.name, max_eval_time, time_to_try_again, save_lineages, batch_size)

        print_log.message("Replace worst collective {}".format(worst_collective_label))

        self.collectives[worst_collective_label] = new_collective
    ...
    def _get_worst_collective(self):
        ...
        for label,pop in self.collectives.items():
            collective_fitness = sum(ind.fitness for ind in pop) / len(pop)
            if collective_fitness < fitness:
                worst_collective = label
